...
for iter=1:length(group)-4,%%%130515%%% 4 instead of 2
...
if ~isempty(group{end-2})
    groups = group{end-2};
    groups_names = group{end-3};    
...
br_names = group{end-1};
boot_vals = group{end};
boot_vals(find(boot_vals==0)) = 1;
branch_size = [0.5:0.1:10.4];

numleaves = get(Tr,'NumLeaves');

for k=1:length(boot_vals)
    i=k+numleaves;
    if strcmp(orientation,'bottom')
        %plot(Y(:,i),X(:,i),['-' internal_color{k}], 'LineWidth', sig_branch_size(ind));
        %plot(Y(:,i),X(:,i),'-','color','b', 'LineWidth', branch_size(boot_vals(k)));
        plot(tr.y(i),tr.x(i),['k' 'o'],'MarkerSize',branch_size(boot_vals(k)),'MarkerEdgeColor','k',...
                    'MarkerFaceColor','k');
    else
        %plot(X(:,i),Y(:,i),['-' internal_color{k}], 'LineWidth', sig_branch_size(ind));
        %plot(X(:,i),Y(:,i),'-','color', 'b', 'LineWidth', branch_size(boot_vals(k)));
        plot(tr.x(i),tr.y(i),['k' 'o'],'MarkerSize',branch_size(boot_vals(k)),'MarkerEdgeColor','b',...
                    'MarkerFaceColor','k');
