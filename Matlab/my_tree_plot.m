function [handles,x,y] = my_tree_plot(tr,colors,scale,varargin)
...
if dispTerminalLabels
    for iter=1:length(group)-2,
        ...
        for i=1:tr.numBranches,
            ind=find(strcmp(group{iter},tr.names{tr.numLeaves+i}));
            if ~isempty(ind),
                set(h.branchNodeLabels(i),'color',terminal_color,'clipping','off')
            end
        end
    end
end
